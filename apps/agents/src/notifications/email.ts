 ...
 * No-op when RESEND_API_KEY or NOTIFICATION_EMAIL is not configured.
 */
export class EmailSender {
  ...
  constructor() {
    ...
    this.recipient = process.env.NOTIFICATION_EMAIL;
  }

  get isConfigured(): boolean {
    return Boolean(this.apiKey && this.recipient);
  }
  ...
  async send(payload: NotificationPayload): Promise<void> {
    ...
    const actionLink =
      payload.actionUrl && isSafeUrl(payload.actionUrl)
        ? `<a href="${escapeHtml(payload.actionUrl)}" style="display: inline-block; padding: 8px 16px; background: #0ea5e9; color: white; text-decoration: none; border-radius: 6px;">View in Dashboard</a>`
        : '';
          ...
          html: `
            <div style="font-family: system-ui, sans-serif; max-width: 600px; margin: 0 auto;">
              <div style="border-left: 4px solid ${severityColor}; padding: 12px 16px; background: #f9fafb; margin-bottom: 16px;">
                ...
                <p style="margin: 0; color: #4b5563;">${safeBody}</p>
              </div>
              ${actionLink}
