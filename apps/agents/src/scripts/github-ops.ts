...
function checkPRs(): Check {
  ...
  const stale = activePRs.filter(
    (p) => daysSince(p.createdAt) >= T.PR_WARN_AGE && !p.reviewDecision,
  );
  ...
  const rating = critical.length > 0 ? 'FAIL' : stale.length > 0 ? 'WARN' : 'PASS';
...
function checkIssues(): Check {
  ...
  if (!Array.isArray(issues)) {
    return {
      name: 'Issues Triage',
      rating: 'SKIP',
      summary: 'Could not fetch issues',
      details: issues,
    };
  }
  ...
  const rating =
    criticalStale.length > 0 || unlabeled.length >= 5
      ? 'FAIL'
...
function checkSecurity(): Check {
  const alerts = ghApi(
    '/dependabot/alerts',
    '[.[] | select(.state=="open") | {number, severity: .security_vulnerability.severity, package: .security_vulnerability.package.name, fix: .security_vulnerability.first_patched_version.identifier}]',
  );
  ...
  const alertList = Array.isArray(alerts)
    ? (alerts as Array<{ number: number; severity: string; package: string; fix: string }>)
    : [];
...
function checkBranches(): Check {
  ...
  const openPRBranches = gh(`pr list --repo ${REPO} --json headRefName`) as Array<{
    headRefName: string;
  }>;
  ...
  if (!Array.isArray(branches)) {
    return {
      name: 'Branch Hygiene',
      rating: 'SKIP',
      summary: 'Could not fetch branches',
      details: branches,
    };
  }
  ...
  const rating =
    orphans.length >= T.BRANCH_FAIL_AGE / 3 ? 'FAIL' : orphans.length > 0 ? 'WARN' : 'PASS';
  ...
  return {
    ...
    details: {
      total: (branches as string[]).length,
      orphaned: orphans,
...
function checkDeployments(): Check {
  ...
  return {
    ...
    actions:
      ageDays > 7 ? ['No deployment in 7+ days — verify Vercel and Railway are healthy'] : [],
  };
}
