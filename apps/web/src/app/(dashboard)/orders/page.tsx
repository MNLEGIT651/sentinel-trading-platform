...
function EntryBadge({ entry }: { entry: TimelineEntry }) {
  ...
  if (entry.type === 'risk') {
    ...
    label = (entry.data as OrderHistoryEntry).status.charAt(0).toUpperCase() +
...
function ExecutionQuality({ order }: { order: OrderHistoryEntry }) {
  ...
  const hasLimitComparison = order.order_type === 'limit' && order.fill_price != null;
  const slippagePct =
    hasLimitComparison && order.fill_price
      ? ((order.fill_price - order.fill_price) / order.fill_price) * 100
      : null;
      ...
      <div className="flex flex-wrap gap-3 text-xs">
        <div>
          ...
          <span className="text-zinc-200 font-medium">
            {formatCurrency(order.fill_price)}
          </span>
        </div>
        {order.order_type !== 'market' && (
          <div>
            <span className="text-zinc-500">
              {order.order_type === 'limit' ? 'Limit' : 'Request'} Price: </span>
...
function OrderLifecycleBar({ status }: { status: string }) {
  ...
  const currentIdx = isTerminal ? -1 : ORDER_STATUS_STEPS.indexOf(status as typeof ORDER_STATUS_STEPS[number]);
          ...
          <div key={step} className="flex items-center gap-1">
            ...
            {idx < ORDER_STATUS_STEPS.length - 1 && (
              <div className={cn(
                ...
                isActive ? 'bg-green-400/30' : 'bg-zinc-800',
              )} />
            )}
          </div>
        );
  ...
  const summary = useMemo(() => {
    ...
    if (entry.type === 'order') {
      ...
      return {
        ...
        secondary: order.fill_price != null
        ...
        {entry.type === 'order' && (
          <div className="mt-2 flex flex-wrap gap-3 text-xs text-zinc-500">
            ...
            {(entry.data as OrderHistoryEntry).filled_qty > 0 && (
              <span>
                Filled: {(entry.data as OrderHistoryEntry).filled_qty}/{(entry.data as OrderHistoryEntry).qty}
              </span>
            )}
