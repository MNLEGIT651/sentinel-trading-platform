...
| A05 Security Misconfiguration | CSP headers, Dependabot, CodeQL, security-safety CI          | `.github/workflows/`, `next.config.ts`                        |
...
| `security-safety.yml`   | Scheduled security + dependency audit | Daily                       |
| `dependency-review.yml` | Block PRs with vulnerable deps        | On dependency changes       |
| `codeql-analysis.yml`   | SAST for JS/TS + Python               | Push to main, PRs, weekly   |
| `secrets-scan.yml`      | Gitleaks secret detection             | Every push/PR               |
| `zap-baseline.yml`      | OWASP ZAP DAST baseline               | On deployment               |
| `container-scan.yml`    | Trivy container image scanning        | Dockerfile changes, weekly  |
| `scorecard.yml`         | OpenSSF repo health score             | Main branch changes, weekly |
| `lighthouse-ci.yml`     | Performance + best practices          | PRs, deployments            |
...
| PW.7 (Security testing)       | CodeQL, Gitleaks, ZAP, Trivy, pnpm audit                  |
