...
- **Guarded auto** — allow auto-merge only when `Policy Verdict` is green and the PR is not escalated.
...
8. **Escalate** — protected-path, auth, schema, or production-deploy changes stay blocked
...
```json
{
  ...
  "required_checks": [{ "name": "CI / Test Web", "status": "green | red | pending | missing" }],
  "specialist_results": [
...
deploy config and requires human sign-off. `ESCALATE` remains blocked until the
PR carries `decision/human-approved`.
